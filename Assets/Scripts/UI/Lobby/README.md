...
| `LobbyUIController.RoomList.cs` | Room list scroll, refresh, join + password dialog |
| `LobbyUIController.Friends.cs` | Friends / Requests tabs, invite, join friend's room |
...
├── PasswordDialog        (optional, phòng private)
...
| Create room inputs | `roomNameInput`, `maxPlayersDropdown`, `passwordInput` |
...
| `passwordDialog` | Optional; null → join private không hỏi password |
...
sequenceDiagram
    ...
    alt Thành công
        ...
        LM-->>UI: CurrentRoomChanged(room)
        UI->>UI: ApplyCurrentRoomUi()
    else Sai password / không tìm thấy
...
- Friends & room list: **mock** qua `LobbyManager`.
- `TryStartMatch` chưa allocate PurrNet / EdgeGap server.
- Steam status trong friends UI là placeholder.
- Password dialog có thể để null (join private sẽ gửi password rỗng).
